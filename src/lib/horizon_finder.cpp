...
namespace floordetection {
    ...
    int HorizonFinder::find(const cv::Mat &blured) {
        ...
        for (unsigned i = 0; i < subimages; i++) {
            foregrounds2[i] = 0;
            cv::Mat subimg = dst(cv::Range(i * height + (height / 2), (i + 1) * height + (height / 2)),
                                 cv::Range::all());
            uchar *ptrData = subimg.ptr<uchar>(0);
            for (int j = 0; j < subimg.rows * subimg.cols; j++, ++ptrData) {
